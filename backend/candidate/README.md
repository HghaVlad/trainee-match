...
- Админ-эндпоинты: просмотр кандидатов и резюме, архивирование резюме, управление справочником навыков.
...
- Admin: административные методы для кандидатов, резюме и навыков.

Полный список ручек:

- `GET /api/v1/admin/candidates`
- `GET /api/v1/admin/candidates/{id}`
- `GET /api/v1/admin/candidates/{id}/resumes`
- `GET /api/v1/admin/resumes/{id}`
- `POST /api/v1/admin/resumes/{id}/archive`
...
Админ-методы также требуют роли администратора в Keycloak.
...
```bash
cd "trainee-match\backend\candidate"
go run .\cmd\main.go
```
