package listcandidatesummary

import (
	"encoding/base64"
	"encoding/json"
	"errors"
)

var (
	ErrInvalidCursor       = errors.New("invalid cursor")
	ErrCursorOrderMismatch = errors.New("cursor order mismatch")
	ErrUnsupportedOrder    = errors.New("unsupported list order")
)

type cursorWrapper struct {
	Order Order           `json:"order"`
	Data  json.RawMessage `json:"data"`
}

func decodeCursor(raw string, expectedOrder Order) (*SummaryCursor, error) {
	if raw == "" {
		//nolint:nilnil // nil cursor is possible
		return nil, nil
	}

	decoded, err := base64.StdEncoding.DecodeString(raw)
	if err != nil {
		return nil, errors.Join(ErrInvalidCursor, err)
	}

	var wrapper cursorWrapper
	if err := json.Unmarshal(decoded, &wrapper); err != nil {
		return nil, errors.Join(ErrInvalidCursor, err)
	}

	if wrapper.Order != expectedOrder {
		return nil, ErrCursorOrderMismatch
	}

	var cursor SummaryCursor
	if err := json.Unmarshal(wrapper.Data, &cursor); err != nil {
		return nil, errors.Join(ErrInvalidCursor, err)
	}

	return &cursor, nil
}

func encodeCursor(order Order, cursor *SummaryCursor) (*string, error) {
	if cursor == nil {
		//nolint:nilnil // nil cursor is possible
		return nil, nil
	}

	payload, err := json.Marshal(cursor)
	if err != nil {
		return nil, err
	}

	wrapper, err := json.Marshal(cursorWrapper{
		Order: order,
		Data:  payload,
	})
	if err != nil {
		return nil, err
	}

	encoded := base64.StdEncoding.EncodeToString(wrapper)
	return &encoded, nil
}
